...
def fit_block_to_vertical_limit(
    ...
    *,
    ...
    for _ in range(10):
        ...
        elif best_leading > (0.52 if is_body else 0.3):
            best_leading = max((0.52 if is_body else 0.3), best_leading - 0.01)
        else:
            break
    ...
    if overflow_ratio > 1.02:
        ...
        floor_leading = 0.52 if is_body else 0.28
        if is_dense_block:
            floor_leading = 0.5 if is_body else 0.24
        if severe_overflow:
            floor_leading = min(floor_leading, 0.44 if is_body else 0.22)
        if extreme_overflow:
            floor_leading = min(floor_leading, 0.38 if is_body else 0.16)
        ...
        for _ in range(18):
            ...
            if best_font > dense_min_font:
                best_font = max(dense_min_font, best_font - (0.22 if severe_overflow else 0.16))
            elif best_leading > floor_leading:
                best_leading = max(floor_leading, best_leading - (0.03 if severe_overflow else 0.02))
            estimated_height = estimated_render_height_pt(inner, protected_text, formula_map, best_font, best_leading)
